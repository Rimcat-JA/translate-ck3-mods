...
3. Confirm the detected or explicitly selected source language, source CK3 locale, target language, target CK3 locale, provider, endpoint, and model. Disable translation when the detected source language already equals the target language. API keys, including an optional authenticated-local-server token, may be stored only in Windows Credential Manager; never place them in scripts, settings JSON, SQLite, logs, manifests, or generated mods.
4. Read [references/ck3-localization.md](references/ck3-localization.md) before changing localization files. Read [references/providers.md](references/providers.md) before configuring a local or remote provider. Read [references/agent-compatibility.md](references/agent-compatibility.md) when packaging or installing the skill for another agent.
...
- For LM Studio model discovery, probe native `/api/v1/models` first, OpenAI-compatible `/v1/models` second, and legacy `/api/v0/models` last. Prefer native type metadata so embedding-only models are excluded. Preserve a manually entered model identifier when discovery is unavailable or empty; JIT loading may make it usable on the first request.
